...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        forward_func = forward_functions[self.cell_type]
        backward_functions = {
            'rnn': rnn_backward,
            'lstm': lstm_backward,
        }
        backward_func = backward_functions[self.cell_type]
        ...
        x, cache_embed = word_embedding_forward(captions_in, W_embed)  # (N, T, W)
        # produce hidden states
        h, cache_h = forward_func(x, h0, Wx, Wh, b)  # (N, T, H)
        # compute scores over the vocabulary
        out, cache = temporal_affine_forward(h, W_vocab, b_vocab)  # (N, T, V)
        loss, dout = temporal_softmax_loss(out, captions_out, mask)

        # compute backward
        dh, dW_vocab, db_vocab = temporal_affine_backward(dout, cache)
        dx, dh0, dWx, dWh, db = backward_func(dh, cache_h)
    ...
    def sample(self, features, max_length=30):
        ...
        # initialize the hidden state by applying the learned affine
        # transform to the input image features
        h0 = features.dot(W_proj) + b_proj
        # First word that should be fed to RNN is self._start
        ...
        for i in range(max_length):
            ...
            if self.cell_type == 'rnn':
                ...
                prev_c = next_c
            prev_h = next_h
            # Apply the learned affine transformation to the next hidden state
