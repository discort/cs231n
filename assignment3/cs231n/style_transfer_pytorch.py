...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    style_loss = torch.zeros(len(style_layers))
    for idx, i in enumerate(style_layers):
        style_loss[idx] = style_weights[idx] * torch.sum((gram_matrix(feats[i]) - style_targets[idx]) ** 2)
    return torch.sum(style_loss)
...
def tv_loss(img, tv_weight):
    ...
    c = x[:, :, 1:]
    d = x[:, :, :(W-1)]
    loss = tv_weight * (torch.sum((a - b) ** 2) + torch.sum((c - d) ** 2))
    return loss
